...
import { resumeCampaign } from "../../../../../lib/broadcast/machine";
...
/**
 * POST /admin/api/email-campaigns/[id]/resume — lanjutkan (paused → queued,
 * diproses ulang worker). 200 { ok:true } · 400 invalid-transition · 404 · 401.
 */
export const POST: APIRoute = async ({ cookies, params, request }) => {
  ...
  const result = await resumeCampaign(params.id ?? "", { adminUserId: admin.id, ip });
