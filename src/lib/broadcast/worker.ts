...
export type BroadcastStopReason =
  | "idle" | "claim-failed" | "minute-limit" | "hour-limit" | "day-limit"
...
async function countDeliveriesForCampaign(campaignId: string, since: Date): Promise<number> {
  const [row] = await db.select({ n: sql<number>`count(*)::int` })
    ...
    .where(and(
      ...
      gte(emailDeliveries.sentAt, since),
    ));
  return row?.n ?? 0;
}
 ...
 */
export async function processBroadcast(opts?: {
  ...
  const [next] = await db.select({ id: emailCampaigns.id })
    .from(emailCampaigns)
  ...
  const stopWith = async (
    reason: "minute-limit" | "hour-limit" | "day-limit",
  ): Promise<BroadcastResult> => {
  ...
  for (const row of rows) {
    ...
    if (!row.html) {
      ...
      await db.update(emailCampaignRecipients)
        .set({ status: "failed" }).where(eq(emailCampaignRecipients.id, row.id));
      await db.insert(emailDeliveries).values({
        contactId: row.contactId, campaignRecipientId: row.id, emailType: "broadcast",
        status: "failed", error: "missing-render", sentAt: now,
      });
      skipped++;
      continue;
    }
    ...
    try {
      ...
      await db.insert(emailDeliveries).values({
        contactId: row.contactId, campaignRecipientId: row.id, providerMessageId,
        emailType: "broadcast", status: "accepted", sentAt: now,
      });
      await db.update(emailCampaignRecipients)
        .set({ status: "sent" }).where(eq(emailCampaignRecipients.id, row.id));
      sent++;
    } catch (err) {
      await db.insert(emailDeliveries).values({
        contactId: row.contactId, campaignRecipientId: row.id, emailType: "broadcast",
        status: "failed", error: String(err).slice(0, 500), sentAt: now,
      });
      await db.update(emailCampaignRecipients)
        .set({ status: "failed" }).where(eq(emailCampaignRecipients.id, row.id));
      skipped++;
