...
import { validateUpload, storeAsset, removeAsset, reorderAssets } from "../../src/lib/admin/assets";
...
describe("asset upload", () => {
  ...
  it("storeAsset rejects invalid upload without writing", async () => {
    ...
    const res = await storeAsset({
      campaignId, filename: "virus.exe", mimeType: "application/x-msdownload",
      body: new ArrayBuffer(4), nameId: "X", sortOrder: 0,
    });
  ...
  it("storeAsset writes asset_uploaded audit; removeAsset writes asset_removed", async () => {
    ...
    const res = (await storeAsset({
      campaignId, filename: "Modul 2026.zip", mimeType: "application/zip",
      body: new TextEncoder().encode("zipbytes").buffer as ArrayBuffer,
      nameId: "Modul 2026", sortOrder: 0,
    }, { adminUserId: null, ip: "1.1.1.1" })) as { ok: true; id: string };
  ...
  it("reorderAssets persists sortOrder by index", async () => {
    ...
    const a = (await storeAsset({
      campaignId, filename: "a.pdf", mimeType: "application/pdf",
      body: new ArrayBuffer(2), nameId: "A", sortOrder: 0,
    })) as { ok: true; id: string };
    const b = (await storeAsset({
      campaignId, filename: "b.pdf", mimeType: "application/pdf",
      body: new ArrayBuffer(2), nameId: "B", sortOrder: 1,
    })) as { ok: true; id: string };
