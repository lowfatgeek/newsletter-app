...
describe("broadcast schema", () => {
  ...
  it("rejects duplicate recipient per campaign and duplicate click token", async () => {
    ...
    const [camp] = await db
      .insert(emailCampaigns)
      .values({ subjectId: "s", bodyHtmlId: "b", audienceFilter: {}, maxPerMinute: 1, maxPerHour: 1 })
      .returning();
    await db
      .insert(emailCampaignRecipients)
      .values({ campaignId: camp.id, contactId: c.id, localeSelected: "id", clickTokenHash: "t1" });
    await expect(
      db
        .insert(emailCampaignRecipients)
        .values({ campaignId: camp.id, contactId: c.id, localeSelected: "id", clickTokenHash: "t2" }),
    ).rejects.toThrow();
    await expect(
      db
        .insert(emailCampaignRecipients)
  ...
  it("round-trips snapshot render and first click columns on recipients", async () => {
    ...
    const [camp] = await db
      .insert(emailCampaigns)
      .values({ subjectId: "s", bodyHtmlId: "b", audienceFilter: { all: true }, maxPerMinute: 1, maxPerHour: 1 })
      .returning();
    ...
    const [r2] = await db
      ...
      .values({
        campaignId: camp.id,
        contactId: (await db.insert(contacts).values({ emailNormalized: "f@gmail.com" }).returning())[0].id,
        localeSelected: "id",
        clickTokenHash: "t4",
      })
      .returning();
